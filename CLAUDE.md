...
- **No section recaps**: A recap after a worked example ("What we can learn from this example:") is trimmed to what the example itself adds, not filled with points the concept sections already made. The same applies to a paragraph that describes in prose what the diagram next to it already shows. The English lead-in is "What this example shows:", not "What we can learn from this example:"; existing entries are switched when touched
- **Cut rhetoric, not facts**: These rules target announcements, motivation and meta-sentences about the entry. A sentence carrying a verifiable statement about notation — the direction of an arrow, the shape of an element, solid versus dashed, a rule about what is forbidden — is rephrased or kept, never dropped because the same fact appears elsewhere. Entries in a "Common Mistakes" list are deliberate duplicates: the mistake perspective carries its own value and stays. The same applies to "Advantages" and "Disadvantages" lists: they evaluate what a "What Is Included" list or a definition describes, so a feature reappearing there as an advantage stays
